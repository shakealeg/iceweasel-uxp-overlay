...
if [[ ${PV} == 9999 ]]; then
	...
	UXP_VER="v2018.06.01"
	IW_VER="1.9"
	SRC_URI="https://github.com/MoonchildProductions/UXP/archive/$UXP_VER.tar.gz"
...
src_unpack() {
	if [[ ${PV} == "9999" ]]; then
		...
		cd "${S}/application" && wget $IW_URI && unpack iceweasel-uxp-$IW_VER.tar.gz || die "Failed to download application source (wget)"
		mv "${S}/application/iceweasel-uxp-$IW_VER" "${S}/application/iceweasel-uxp" || die "Failed to remove version from application name (broken branding)"
	fi
}
